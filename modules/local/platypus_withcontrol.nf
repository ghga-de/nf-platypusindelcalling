...
process PLATYPUS_WITHCONTROL {
    ...
    output:
    tuple val(sample), path('*.platypus.withcontrol.vcf')                      , emit: withcontrol_vcf
    tuple val(sample), path('*.platypus.withcontrol.log')                      , emit: platypus_log
    ...
    def out_vcf = "${sample}.platypus.withcontrol.vcf"
    def out_log = "${sample}.platypus.withcontrol.log"
